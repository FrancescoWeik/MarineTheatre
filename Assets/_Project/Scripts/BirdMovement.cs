...
public class BirdMovement : MonoBehaviour, IPointerClickHandler
{
    [SerializeField] private FishGeneralDataScriptable birdData;
    ...
    {
        bool wasClicked = SaveSystem.GetItemClicked(birdData.title);
        if (!wasClicked)
        {
            SaveSystem.SetItemClicked(birdData.title, true);
